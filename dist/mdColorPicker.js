...
(function (window, angular, undefined) {
	...
	function GradientCanvasFactory() {
		...
		return function gradientCanvas(type, shared) {
			var canvas = new GradientCanvas(type, type != 'spectrum', shared);
	...
	function GradientCanvas(type, restrictX, $rootScope) {
		...
		this.get = angular.bind(this, function ($temp_scope, $temp_element, $rootScope, $temp_attrs) {
			////////////////////////////
			// Variables
			////////////////////////////
	...
	GradientCanvas.prototype.setMarkerCenter = function (x, y) {
		...
		if (y === undefined) {
			...
			xFinal = (Math.max(Math.min(this.height + xOffset, xAdjusted), xOffset));
			yFinal = (Math.max(Math.min(this.height + yOffset, yAdjusted), yOffset));
			// Debug output
	...
	angular.module('mdColorPicker', [])
			...
			return {
				...
				controller: ['$scope', '$element', '$attrs', '$mdDialog', '$mdColorPicker', function ($scope, $element, $attrs, $mdDialog, $mdColorPicker) {
					...
					if ($scope.options !== undefined) {
						...
						if ($scope.mdColorChange)
							$scope.options.mdColorChange = $scope.mdColorChange;
					}


					var triggerOnChange = function (newValue) {
						if ($scope.mdColorChange)
							$scope.mdColorChange({
								$color: newValue
							});
					...
					var triggerOnSelect = function (newValue) {
						if ($scope.mdColorSelect)
							$scope.mdColorSelect({
								$color: newValue
							});
					}

					var triggerOnClear = function () {
						if ($scope.mdColorClear)
							$scope.mdColorClear($scope.value);
					}

					$scope.$on('mdColorPicker:spectrumColorChange', function (event, args) {
						triggerOnChange('#' + tinycolor(args.color).toHex());
					});

					$scope.$on('mdColorPicker:colorSet', function (event, args) {
						triggerOnSelect(args.color._originalInput);
					})
					...
					$scope.clearValue = function clearValue() {
						...
						$scope.$emit('mdColorPicker:colorCleared');
					};
					$scope.showColorPicker = function showColorPicker($event) {
						...
						$mdColorPicker.show({
							...
							mdColorChange: triggerOnChange,
							mdColorChangeHistory: triggerOnSelect,

							$event: $event,

						}).then(function (color) {
							$scope.value = color; // Called on "Select"
						});
					};
				}],
		...
		.directive('mdColorPickerContainer', ['$compile', '$timeout', '$mdColorPalette', '$rootScope', 'mdColorPickerHistory', function ($compile, $timeout, $mdColorPalette, $rootScope, colorHistory) {
			return {
				...
				controller: ["$scope", "$element", "$attrs", "$compile", function ($scope, $element, $attrs, $compile) {
					...
					$scope.setValue = function setValue() {
						...
						if ($scope.color && outputFn[$scope.type] && $scope.color.toRgbString() !== 'rgba(0, 0, 0, 0)') {
							$scope.value = $scope.color[outputFn[$scope.type]]();
							// $rootScope.$broadcast('mdColorPicker:colorChange', {
							// 	newValue: $scope.value
							// });
							$scope.$parent.mdColorChange($scope.value);
						}
					};
		...
		.directive('mdColorPickerSpectrum', ['mdColorGradientCanvas', '$rootScope', function (mdColorGradientCanvas, $rootScope) {
			return new mdColorGradientCanvas('spectrum', $rootScope);
		}])
				...
				show: function (options) {
					...
					dialog = $mdDialog.show({
						...
						controller: ['$scope', 'options', function ($scope, options) {
							...
							$scope.mdColorChange = !options.mdColorChange ? function() {} : options.mdColorChange;
							onHistoryChangeCallback = $scope.mdColorChangeHistory = !options.mdColorChangeHistory ? function() {} : options.mdColorChangeHistory;
